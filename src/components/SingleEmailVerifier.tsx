...
import { simulateEmailVerification, suggestEmailCorrections, commonTypos } from "@/lib/emailValidation";
import { Mail, Loader2, CheckCircle, XCircle, AlertTriangle, Zap, Shield, Activity, Wand2 } from "lucide-react";
...
const SingleEmailVerifier = ({ onResult }: SingleEmailVerifierProps) => {
  ...
  const verifyEmail = async () => {
    ...
    // Simulate verification steps with real analysis
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-semibold mb-2">Single Email Verification</h2>
        <p className="text-muted-foreground">
          Advanced email verification with reputation scoring and domain health analysis
        </p>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-4 gap-3 text-center">
              <div className={`p-3 rounded-lg transition-all duration-500 ${
                currentStep >= 0 ? 'bg-success/20 text-success' : 'bg-muted/20'
              }`}>
              ...
              <div className={`p-3 rounded-lg transition-all duration-500 ${
                currentStep >= 2 ? 'bg-success/20 text-success' : 'bg-muted/20'
              }`}>
              ...
              <div className={`p-3 rounded-lg transition-all duration-500 ${
                currentStep >= 4 ? 'bg-warning/20 text-warning' : 'bg-muted/20'
              }`}>
              ...
              <div className={`p-3 rounded-lg transition-all duration-500 ${
                currentStep >= 6 ? 'bg-primary/20 text-primary' : 'bg-muted/20'
              }`}>
